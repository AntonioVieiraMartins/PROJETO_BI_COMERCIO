USE COMERCIO_OLTP
GO

TRUNCATE TABLE ITEM_NF
GO

DECLARE
			@ID_PRODUTO INT,
			@ID_NF INT,
			@QUANTIDADE INT,
			@VALOR NUMERIC(10,2),
			@TOTAL NUMERIC(10,2)
BEGIN

			SET @ID_PRODUTO = 
			(SELECT TOP 1 IDPRODUTO FROM PRODUTO ORDER BY NEWID())
			SET @ID_NF = 
			(SELECT TOP 1 IDNF FROM NOTA_FISCAL ORDER BY NEWID())
			SET @QUANTIDADE = (SELECT ROUND(RAND()* 3 + 1, 0))
			SET @VALOR = (SELECT VALOR FROM PRODUTO
			        	  WHERE IDPRODUTO = @ID_PRODUTO)
			SET @TOTAL = @QUANTIDADE * @VALOR

			INSERT INTO ITEM_NF(ID_PRODUTO,ID_NF,QUANTIDADE,TOTAL)
			 VALUES
			(@ID_PRODUTO,@ID_NF,@QUANTIDADE,@TOTAL)
END
GO 26000

SELECT TOP 1 * FROM NOTA_FISCAL
SELECT TOP 1 * FROM ITEM_NOTA
SELECT * FROM ITEM_NOTA

/* VERIFICANDO AS NOTAS QUE FICARAM SEM ITENS */

SELECT IDNOTA FROM NOTA_FISCAL
WHERE IDNOTA NOT IN(SELECT ID_NOTA_FISCAL FROM
ITEM_NOTA)
GO

SELECT * FROM ITEM_NOTA
GO

/* CRIANDO UM CURSOR PARA PREENCHER AS NOTAS QUE NÃO TIVERAM ITENS */

CREATE PROCEDURE CAD_NOTAS AS

DECLARE C_NOTAS CURSOR FOR
	SELECT IDNF FROM NOTA_FISCAL
	WHERE IDNF NOT IN(SELECT ID_NF FROM
	ITEM_NF)	
	
DECLARE @IDNOTA INT,
		@ID_PRODUTO INT,
		@VALOR DECIMAL(10,2), 
		@TOTAL DECIMAL(10,2)	    
		
OPEN C_NOTAS

FETCH NEXT FROM C_NOTAS
INTO @IDNOTA

WHILE @@FETCH_STATUS = 0
BEGIN
	
			SET @ID_PRODUTO = 
			(SELECT TOP 1 IDPRODUTO FROM PRODUTO ORDER BY NEWID())
			SET @VALOR = (SELECT VALOR FROM PRODUTO
			        	  WHERE IDPRODUTO = @ID_PRODUTO)
			SET @TOTAL = @VALOR
	
			INSERT INTO ITEM_NF(ID_PRODUTO,ID_NF,QUANTIDADE,TOTAL)
			 VALUES(@ID_PRODUTO,@IDNOTA,1,@TOTAL)
		
	FETCH NEXT FROM C_NOTAS
	INTO @IDNOTA
	
END

CLOSE C_NOTAS
DEALLOCATE C_NOTAS
GO

EXEC CAD_NOTAS
GO

/* CRIANDO UMA VIEW PARA VERIFICAR OS ITENS PEDIDOS */

CREATE VIEW V_ITEM_NOTA AS
SELECT ID_NF AS "NOTA FISCAL",
	   PRODUTO, 
	   VALOR, 
       QUANTIDADE,
	   TOTAL AS "TOTAL DO ITEM"
FROM PRODUTO
INNER JOIN ITEM_NF
ON IDPRODUTO = ID_PRODUTO
GO

SELECT * FROM ITEM_NF
SELECT * FROM V_ITEM_NOTA
ORDER BY 1
GO

SELECT * FROM NOTA_FISCAL

SELECT C.NOME, C.SOBRENOME,C.SEXO,N.DATA,N.IDNF,P.PRODUTO,
N.TOTAL
FROM CLIENTE C
INNER JOIN NOTA_FISCAL N
ON C.IDCLIENTE = N.ID_CLIENTE
INNER JOIN ITEM_NF I
ON N.IDNF = I.ID_NF
INNER JOIN PRODUTO P
ON P.IDPRODUTO = I.ID_PRODUTO
ORDER BY 5
GO